...
from MIRS.mirs.src.mirs_controller.mirs_controller.trajectory.trajectory_generator import Trajectory
import uuid
import time
...
class SimpleObject:
    ...
    def __init__(self, name, dim=3, pos=(0,0,0),shape='cube', mass=5, length=0.1, width=0.1, height=0.1, gravity=-9.81, coef_res=0.9):
        self.name = name
        ...
        self.mass = mass
        self.position = np.array(pos,ndmin=2)
...
class Simulation(RoboPlotter):
    ...
    def create_objects(self):
        for obj in self.world.get_objects():
            ...
            props = obj[1]
            # Create axis
            # axes = [props.position, props.width, props.height]
            # axes = [1,1,1]
            # # data = np.ones(axes, dtype='bool')
            # data = np.ones(axes)
            obj_plt = self.ax.scatter(props.position[0][0],props.position[0][1],props.position[0][2],s=100)
            self.plots[id] = obj_plt
