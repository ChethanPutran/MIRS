...
from mirs_interfaces.msg import CameraState
...
class Camera(Node):
    ...
    ***** Calibration params (f, b, u0, v0) *****
    ...
    def __init__(self, size=100):
        ...
        self.camera_state_publisher = self.create_publisher(TOPICS.TOPIC_CAMERA_STATE,CameraState,1)
        self.create_subscription(TOPICS.TOPIC_CAMERA_STATE_RECORDING,String,self.record_callback)
        self.create_subscription(TOPICS.TOPIC_CAMERA_STATE_LIVE,String,self.live_capture_callback)
        self.state = ''
        self.recording = None
    ...
    def record_callback(self,msg):
        if msg.cmd == "START_RECORDING":
            self.set_state("START_RECORD")
            self.record()
        elif msg.cmd == "END_RECORDING":
            self.set_state("END_RECORD")
            self.send_recording()
    
    def record(self):
        recording = None
        while not(self.get_state() == "END_RECORD"):
            pass
            #frame = self.cap.read()
            #record code
    ...
    def publisher(self,msg):
        self.camera_state_publisher.publish(msg)
    ...
    def live_capture_callback(self,msg):
        if msg.cmd=="START_LIVE_CAPTURE":
            self.set_state("LIVE_CAPTURE")
            self.live_capture()
        elif msg.cmd == "END_LIVE_CAPTURE":
            self.set_state("END_CAPTURE")

    def live_capture(self):
        while not(self.get_state() == "END_CAPTURE"):
            pass
            #frame = self.cap.read()
            #record code
